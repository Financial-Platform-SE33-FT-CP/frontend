...
export default function BillForm({
  ...
  function validate(): string | null {
    ...
    if (dueDate < issueDate)
      return "Due date cannot be earlier than bill date.";
    if (lines.length < 1) return "At least one bill line is required.";
    ...
    <>
      <form onSubmit={handleSaveDraft} className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>
              {mode === "create" ? "New bill" : "Edit draft bill"}
            </CardTitle>
            <CardDescription>
              Save as draft, then record to post Dr Expense / Dr GST Input / Cr
              AP.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-3">
            <div className="space-y-2 sm:col-span-1">
              ...
              <VendorSelect
                ...
                disabled={!canEdit || saving || recording}
              />
            </div>
            ...
            {lines.map((line, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="md:col-span-2 space-y-1">
                    ...
                    <ExpenseAccountPicker
                      ...
                      onChange={(id) =>
                        setLines((p) =>
                          p.map((l) =>
                            l.key === line.key ? { ...l, account_id: id } : l,
                          ),
                        )
                      }
                    />
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium">GST code</label>

                    <GstCodeSelect
                      value={line.gst_code_id}
                      codes={gstCodes}
                      allowedKinds={["input", "zero_rated", "exempt"]}
                      disabled={!canEdit || saving || recording}
                      onChange={(code) => {
                        setLines((previous) =>
                          previous.map((item) =>
                  ...
                  <div className="md:col-span-6 flex items-center justify-between text-sm text-muted-foreground">
                    <span>
                      Line {idx + 1}: {formatMoney(preview.line_total)} + GST{" "}
